// Twitter/X CA Mentions — SocialData.tools API
...
export default async function handler(req, res) {
  ...
  if (!API_KEY) {
    return res.status(503).json({ error: 'Twitter search not configured — SOCIALDATA_API_KEY missing' });
  }
  ...
  try {
    ...
    if (!searchRes.ok) {
      ...
      return res.status(502).json({ error: 'Twitter search failed' });
    }

    const searchData = await searchRes.json();
    const rawTweets = searchData.tweets || [];

    // Normalize SocialData tweet fields to our consistent shape
    const tweets = rawTweets.slice(0, 20).map((t) => ({
