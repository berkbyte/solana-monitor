...
async function polyFetch(endpoint: 'events' | 'markets', params: Record<string, string>): Promise<Response> {
  ...
  return fetch(`https://solanaterminal.app/api/polymarket?${proxyQs}`);
}

const GEOPOLITICAL_TAGS = [
  'crypto', 'defi', 'bitcoin', 'ethereum', 'solana',
  'economy', 'fed', 'inflation', 'tech', 'ai',
];
...
const EXCLUDE_KEYWORDS = [
  ...
  'ukraine', 'russia', 'china', 'north korea', 'middle-east', 'war',
  'military', 'invasion', 'nato', 'missile', 'sanctions',
  'election', 'president', 'congress', 'senate', 'parliament', 'prime minister',
];

// Crypto-relevant filtering keywords - items must contain at least one
const CRYPTO_RELEVANCE = [
  'bitcoin', 'btc', 'ethereum', 'eth', 'solana', 'sol', 'crypto',
  'blockchain', 'defi', 'token', 'coin', 'nft', 'web3', 'stablecoin',
  'usdc', 'usdt', 'tether', 'binance', 'coinbase', 'etf', 'sec',
  'fed', 'interest rate', 'inflation', 'gdp', 'recession', 'tariff',
  'ai', 'openai', 'nvidia', 'tesla', 'apple', 'tech', 'stock',
  'price', 'market', 'trading', 'exchange',
];
...
function isCryptoRelevant(title: string): boolean {
  const lower = title.toLowerCase();
  return CRYPTO_RELEVANCE.some(kw => lower.includes(kw));
}
  ...
  return breaker.execute(async () => {
    ...
    for (const events of eventResults) {
      for (const event of events) {
        ...
        // Must be crypto/market-relevant
        const eventIsCrypto = isCryptoRelevant(event.title);
        const marketsAreCrypto = (event.markets ?? []).some(m => isCryptoRelevant(m.question ?? ''));
        if (!eventIsCrypto && !marketsAreCrypto) continue;
