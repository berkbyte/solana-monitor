const DEFAULT_REMOTE_HOSTS: Record<string, string> = {
  tech: 'https://solanaterminal.app',
  full: 'https://solanaterminal.app',
  world: 'https://solanaterminal.app',
};
...
export function getRemoteApiBaseUrl(): string {
  ...
  return DEFAULT_REMOTE_HOSTS[variant] ?? DEFAULT_REMOTE_HOSTS.full ?? 'https://solanaterminal.app';
}
...
function isAppOriginUrl(urlStr: string): boolean {
  try {
    ...
    return APP_HOSTS.has(host) || host.endsWith('.solanaterminal.app');
  } catch {
    return false;
  }
}
