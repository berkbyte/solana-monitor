...
import { analysisWorker, enrichWithVelocityML, activityTracker, generateSummary } from '@/services';

// Stubs for removed geo-related services (related-assets.ts has geo config deps)
const MAX_DISTANCE_KM = 500;
function getClusterAssetContext(_cluster: any): any { return { assets: [], summary: '' }; }
function getAssetLabel(_asset: any): string { return ''; }
...
export class NewsPanel extends Panel {
  ...
  private renderClusterHtml(
      ...
      ? `
        <div class="related-assets" data-cluster-id="${escapeHtml(cluster.id)}">
          ...
          <div class="related-assets-list">
            ${assetContext.assets.map((asset: { id: string; type: string; name: string; distanceKm: number }) => `
    ...
    return `
      <div class="${itemClasses}" data-cluster-id="${escapeHtml(cluster.id)}" data-news-id="${escapeHtml(cluster.primaryLink)}">
        <div class="item-source">
          ${tierBadge}
