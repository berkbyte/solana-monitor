...
fn build_app_menu(handle: &AppHandle) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    let about_metadata = AboutMetadata {
        name: Some("Solana Terminal".into()),
        version: Some(env!("CARGO_PKG_VERSION").into()),
        copyright: Some("\u{00a9} 2025".into()),
        website: Some("https://solanaterminal.app".into()),
        website_label: Some("solanaterminal.app".into()),
        ..Default::default()
    };
    let about_item = PredefinedMenuItem::about(handle, Some("About Solana Terminal"), Some(about_metadata))?;
...
fn handle_menu_event(app: &AppHandle, event: tauri::menu::MenuEvent) {
    match event.id().as_ref() {
        ...
        MENU_HELP_GITHUB_ID => {
            let _ = open_in_shell("https://github.com/solana-terminal/solana-terminal");
        }
        MENU_HELP_DEVTOOLS_ID => {
